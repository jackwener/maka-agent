# 06 — Alma Whisper voice transcription: local + cloud + IPC decode
...
detection in this call path (or at least alma doesn't request
...
3. **Adopt the 3-tier dispatch shape EVEN with only cloud
   ...
   matches alma's shape. When Maka adds local Whisper later,
...
- whisper.cpp via `@fugood/whisper.node` supports streaming
  (continuous transcription). Does alma use it, or only
...
- The `whisper-1` cloud model is OpenAI's older one. `gpt-4o-
  ...
  with better quality + language detection. Does alma have a
  migration plan?
...
- Round 3: [`05-readability-execution-context.md`](../alma-deep-dive-yuejing-round-3/05-readability-execution-context.md)
