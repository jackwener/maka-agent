# 08 — Alma MCP client (full server lifecycle + Resources API)
...
Alma's MCP client is a real production-grade implementation:

- Config file at `~/.config/alma/mcp.json` (`main.js:25219-25224`).
...
const Tf = new (class extends Gt {            // Gt = EventEmitter
  ...
  configPath;                                  // ~/.config/alma/mcp.json
  initialized = false;
...
- **Singleton via IIFE class.** Module-level effect — alma boots
...
async startServer(t, n) {
  ...
  if (jn(n)) {                                 // is stdio config?
    ...
    i = new qt({ name: "alma", version: "1.0.0" });
    const e = async (e, t) => {
...
3. Spawn the client (`new qt({ name: "alma", version: "1.0.0" })`).
...
alma handles the token exchange flow and reconnects on success.
This is the same OAuth machinery Maka already has for Claude /
Codex / Cursor, just plugged into the MCP transport layer.

## Resources API

`main.js:25804-25864` (excerpt, two tools alma exposes to the
model):
...
2. **Per-server stderr buffer for diagnostics.** Maka users will
   ...
   MCP feature. Match alma's pattern verbatim.
...
- Does alma's MCP service ever hot-rebuild the pre-loop tool
  selector prompt when servers add tools mid-session? Or does it
  require a session restart?
- How does alma handle MCP tools that the same name as a built-in
...
- Round 1: [`07-borrowable-checklist.md`](../alma-deep-dive-yuejing-2026-05-31/07-borrowable-checklist.md)
...
- Round 1: [`08-extended-topics.md`](../alma-deep-dive-yuejing-2026-05-31/08-extended-topics.md)
