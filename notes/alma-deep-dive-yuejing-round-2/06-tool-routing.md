# 06 — Alma tool routing: pre-loop selector + in-loop ToolSearch
...
Alma runs **two independent LLM-driven routers** to decide which
...
- For ANY web/internet task, **must include BOTH `WebSearch` AND
  `WebFetch`** — alma is opinionated about this.
...
- When asked about Alma itself, route to `Task` (with `alma-guide`
  subagent) + `TaskOutput`.
...
3. **Adopt alma's "always pair X with Y" rules in the selector
