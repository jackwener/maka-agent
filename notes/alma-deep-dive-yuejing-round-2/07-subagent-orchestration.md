# 07 — Alma subagent orchestration (Task / TaskOutput / harness)
...
Alma's subagent system is **not** just "spawn an agent and wait." It
...
> - `agent_id`: pick a managed specialist profile configured in Alma;
...
| `alma-guide` | Answer questions about Alma itself | Glob, Grep, Read, WebFetch, WebSearch |
| `coder` | Write/fix/refactor code | Bash, Glob, Grep, Read, Edit, Write, Skill |
| `alma-operator` | Read/modify Alma config (settings, providers, models) via local API | Bash, Read |

Notice `alma-guide` and `alma-operator` are **self-referential** —
agents that read/modify the host app's own state. This is how alma
...
conversation — e.g., `alma-guide` answering follow-ups. The agent
...
flow, but it's user-visible and one-shot. The alma harness is an
...
context, and alma has to anti-prompt it explicitly.
...
- Renderer surfaces TaskCreate / TaskUpdate / TaskList as Maka tools
  for tracking todo state — different from alma's Task (which
  spawns an agent).
- No `resume`, no `run_in_background`, no harness.

Ranked Maka improvements:

1. **Adopt alma's Task / TaskOutput / TaskStop tool family.** This
...
4. **Add `alma-guide`-equivalent specialist.** A "Maka guide"
...
- Round 1: [`01-agent-loop.md`](../alma-deep-dive-yuejing-2026-05-31/01-agent-loop.md)
