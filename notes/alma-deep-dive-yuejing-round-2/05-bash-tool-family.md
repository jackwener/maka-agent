# 05 — Alma Bash tool family (Bash / BashOutput / KillShell)
...
Alma exposes Bash as a **family** of complementary tools, not a single
`Bash`:
...
- If AsyncLocalStorage context is lost (process boundary issue),
  alma falls back to a single-active-thread heuristic, and warns —
...
2. **Add `BashOutput(shellId)` and `KillShell(shellId)` tools.**
   ...
   background work. Match alma's tool descriptions verbatim so the
...
be ported almost directly from alma's `Jp` / `Vp` patterns.

## Open questions for round 3 of round-2

- Where does alma's background shell registry live? It's referenced
...
- Is there a "no commands at all" mode for fully sandboxed runs?
  Maka probably doesn't need it, but worth knowing alma's stance.
