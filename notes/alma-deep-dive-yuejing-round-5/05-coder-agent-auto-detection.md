# 05 — Alma coder-agent auto-detection: `coderAgentProviderId` resolution
...
distinctly (cross-ref round-4 [`07-websocket-sync.md`](../alma-deep-dive-yuejing-round-4/07-websocket-sync.md)
...
3. **`~/.local/share/mise/installs/node/22.22.0/bin/claude`** —
   ...
   writing. This is alma author's own dev setup leaking into
   the code.
...
most cases, but only if claude is on `PATH` in the alma process's
inherited environment.

Open question: how does alma boot? If it's launched from Finder,
...
And round-3 [`04-permissions-runtime-risk.md`](../alma-deep-dive-yuejing-round-3/04-permissions-runtime-risk.md)
...
1. **Multi-path CLI detection with `which` fallback.** The
   alma pattern (3 hardcoded paths + `which`) is straight-up
...
- The mise path with Node 22.22.0 is fragile. Does alma have a
