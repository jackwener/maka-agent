...
| 05 | `05-coder-agent-auto-detection.md` | 3 sentinel values + DB provider id dispatch + 4-tier Claude Code CLI detection (3 hardcoded paths including mise + `which` fallback) + asymmetric silent-vs-warn fallback (`__auto__` silent, `__claude_code__` warns) + in-thread ⚠️ system messages instead of toasts + `subagent:` sessionId prefix as cross-subsystem convention | **shipped** |
| 06 | `06-telemetry-sentry.md` | Sentry @electron/main init at startup with environment + release tagging + `tracesSampleRate: 0.1` + dev escape hatch via `SENTRY_DEBUG=true` env var + `beforeSend` credential redaction (Authorization + X-Api-Key) + clarifies `Gt()` local alias is `broadcastThreadSync` NOT telemetry (corrects round-4 02) + redaction-gap audit (Cookie/body fields/URL params not covered) | **shipped** |
