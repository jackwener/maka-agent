# 06 — Alma telemetry: Sentry error reporting + WS broadcasts (clarifying the `Gt()` confusion)
...
Alma has **two distinct event systems** that look superficially
similar:
...
| **Sentry telemetry** | Error / crash / performance reporting | sentry.io ingest | post-hoc debugging by alma team |
...
### 1. DSN identifies the alma project

`o441417` is the Sentry organization id; `4510488586485760` is
the project id. Errors go to alma's own dashboard. If Maka
adopts Sentry, **regenerate the DSN** — otherwise Maka errors
land in alma's bucket.

### 2. `release` ties errors to app version

`alma@${app.getVersion()}` — Sentry can group errors by
...
3. **Separate WS broadcast vocabulary from telemetry
   vocabulary.** Alma's `Gt()` local alias caused me to
...
5. **Extend redaction beyond headers.** `Cookie`,
   ...
   be in the redaction list. The alma redaction is a 5-minute
